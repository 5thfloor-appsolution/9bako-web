		...
		if ( empty( $this->frontend_error_messages ) ) {
			...
			if ( ! apply_filters( 'pt-ocdi/disable_pt_branding', false ) ) {
				...
				$response['message'] .= sprintf(
					__( '%1$s%6$sWasn\'t this a great One Click Demo Import experience?%7$s Modidied by %3$sTheme Freesia%4$s. %2$s%5$sClick to Rate us!%4$s%8$s', 'pt-ocdi' ),
					'<div class="notice  notice-info"><p>',
					'<br>',
					'<strong><a href="https://www.themefreesia.com/" target="_blank">',
					'</a></strong>',
					'<strong><a href="' . add_query_arg( 'status', urlencode( $twitter_status ), 'https://wordpress.org/support/theme/pixgraphy/reviews/' ) . '" target="_blank">',
					'<strong>',
					'</strong>',
					'</p></div>'
				);
